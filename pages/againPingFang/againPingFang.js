// pages/againPingFang/againPingFang.js
...
Page({
  ...
  //获取房产性质
  getHouseNature: function (idx, nature) {
    sendRequestApi.getHouseNatureApi().then(res => {
      if (res.data.code == 0) {
        ...
        let natureIdx = 0;
        let valueList = this.data.deedData.assessHouseModels[0].natureValueList;
        let natureIndex = "deedData.assessHouseModels[" + idx + "].natureIndex";

        for (let e = 0; e < valueList.length; e++){
          if (nature == valueList[e]){
              natureIdx = e;
              this.setData({
                [natureIndex]: natureIdx
              });
            }
          }
      }
  ...
  getDetail: function (params){
    ...
    sendRequestApi.getPingFangDetailApi(params).then(res => {
      ...
      if (res.data.code == 0) {
        if (res.data.dqbHouseLog){
          this.setData({
            dqbHouseLog: res.data.dqbHouseLog
          });
        }
        
        let { data: { houseList } } = res;
        ...
        this.setData({
          isShowPage: true,
          [deedData]: deedList,
          houseListLength: houseList.length
        });

        for (let j = 0; j < houseList.length; j++) {
          let imgArr = [];
          let imgObj = {};
          let regionName = "deedData.assessHouseModels[" + j + "].regionName";
          let address = "deedData.assessHouseModels[" + j + "].address";
          let houseNature = "deedData.assessHouseModels[" + j + "].houseNature";
          let natureIndex = "deedData.assessHouseModels[" + j + "].natureIndex";
          let isShowNatureDefault = "deedData.assessHouseModels[" + j + "].isShowNatureDefault";
          let estatesNo = "deedData.assessHouseModels[" + j + "].estatesNo";
          let houseArea = "deedData.assessHouseModels[" + j + "].houseArea";
          let areaRemark = "deedData.assessHouseModels[" + j + "].areaRemark";
          let province = "deedData.assessHouseModels[" + j + "].province";
          let city = "deedData.assessHouseModels[" + j + "].city";
          let district = "deedData.assessHouseModels[" + j + "].district";
          let isShowDefaultProvince = "deedData.assessHouseModels[" + j + "].isShowDefaultProvince";
          let floorNo = "deedData.assessHouseModels[" + j + "].floorNo";
          let floorSum = "deedData.assessHouseModels[" + j + "].floorSum";
          let remark = "deedData.assessHouseModels[" + j + "].remark";
          let urlId = "deedData.assessHouseModels[" + j + "].urlId";
          let assessPrice = "deedData.assessHouseModels[" + j + "].assessPrice";
          let imgList = "deedData.assessHouseModels[" + j + "].imgList";

          for (let n = 0; n < houseList[j].picUrls.length; n++){
            imgObj = {
              imageSrc: util.ServerConfig.resource + "/file/preview.ashx?GUID=" + houseList[j].picUrls[n],
              // imageSrc: "http://testfile.91zhaiquan.net:31102/file/preview.ashx?GUID=" + houseList[j].picUrls[n],
              fileGuid: houseList[j].picUrls[n],
              isShowDelete: true
            }
            imgArr.push(imgObj);
          }
          ...
          //获取房产性质
          this.getHouseNature(j, houseList[j].houseNature);

          this.setData({
            isShowPage: true,
            [houseNature]: houseList[j].houseNature,
            // [natureIndex]: natureIdx,
            [isShowNatureDefault]: false,
            [estatesNo]: houseList[j].estatesNo,
            [houseArea]: houseList[j].houseArea,
            [areaRemark]: houseList[j].areaRemark,
            [province]: houseList[j].province,
            [city]: houseList[j].city,
            [district]: houseList[j].district,
            [isShowDefaultProvince]: false,
            [floorNo]: houseList[j].floorNo,
            [floorSum]: houseList[j].floorSum,
            [remark]: houseList[j].remark,
            [urlId]: houseList[j].urlId,
            [assessPrice]: houseList[j].assessPrice,
            [regionName]: houseList[j].regionName,
      ...
      success: res => {   //成功则返回图片的本地文件路径列表 tempFilePaths
        ...
        wx.uploadFile({
          ...
          success: res => {
            ...
            let imgDistinguishPath = util.ServerConfig.resource + "/file/download.ashx?GUID=" + JSON.parse(res.data)[0].fileGUID;
            let idx = e.currentTarget.dataset.index;
            ...
            //房产证自动识别
            sendRequestApi.deedDistinguishApi(imgDistinguishPath).then(res => {
              if (res != null || res != "") {
                wx.hideLoading();
  ...
  reduceDeed: function () {
    ...
    if (houseList.length <= this.data.houseListLength) {
      this.setData({
        isShowReduce: true
      })
    }
